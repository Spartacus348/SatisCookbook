...
use crate::objects::process::Process;
use crate::recipebook;
...
pub(crate) fn generate_possibilities<'a>(
    ingredient: &Part,
    rate_per_min: f32,
    upline: Vec<Process<'a>>,
) -> Multiverse<'a> {
        ...
        .filter_map(|recipe| {
            ...
            Some(ProductionNode {
                ...
                sources: recipe
                    ...
                    .iter()
                    .map(|&(part, _)| {
                        let part_per_min: f32 =
                            scale_factor * recipe.get_input_rate_per_min(&part).unwrap();
                        (
                            part,
                            generate_possibilities(&part, part_per_min, new_upline.clone()),
                        )
                    })
                    .collect::<BookOfPaths>(),
            })
...
impl OnePath<'_> {
    pub(crate) fn get_power(self: &Self) -> f32 {
        self.source_recipe.building.get_power() as f32 * self.amount
            + self
                .inputs
                .iter()
    ...
    pub(crate) fn get_raw_resources(self: &Self) -> HashMap<Part, f32> {
        let mut raw_resources: HashMap<Part, f32> = self
            .source_recipe
            .building
            .get_input()
            .iter()
        ...
        self.inputs
            .iter()
            .for_each(|(_, path)| update_counter(&mut raw_resources, path.get_raw_resources()));
        raw_resources
    }

    pub(crate) fn count_buildings(self: &Self) -> usize {
        self.amount.ceil() as usize
            + self
                .inputs
                .iter()
...
pub(crate) fn walk_one_path<'a>(
    options: &'a Multiverse<'a>,
    algo: OptimizationMode,
) -> Option<OnePath<'a>> {
...
fn fewest_buildings<'a>(p0: &'a Multiverse<'a>) -> Option<OnePath<'a>> {
    ...
    p0.iter()
        ...
        .reduce(|p0, p1| {
            if p1.count_buildings() < p0.count_buildings() {
                p1
            } else {
                p0
            }
...
fn least_resources<'a>(p0: &'a Multiverse<'a>) -> Option<OnePath<'a>> {
    ...
    p0.iter()
        ...
        .reduce(|p0, p1| {
            if p1.get_raw_resources().values().sum::<f32>()
                < p0.get_raw_resources().values().sum::<f32>()
            {
                p1
            } else {
                p0
